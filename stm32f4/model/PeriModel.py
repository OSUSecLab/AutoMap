...
def start_gdb(filename):
    ...
    gdb.execute('mon flash probe 0')
    gdb.execute('mon flash protect 0 0 127 off')
...
def build_model(peri_addr, value):
    ...
    init_peri_val = init_read_memory(peri_addr, 4, "temp")
    ...
    prev_first_mem = read_memory(base_addr, length, "temp1")
    prev_second_mem = read_memory(0x50000000, 0x40000, "temp2")
    prev_third_mem = read_memory(0xe0000000, 0x10000, "temp3")
    prev_fourth_mem = read_memory(0xe0040000, 0x2000, "temp4")
    prev_fifth_mem = read_memory(0xf0000000, 0x10000, "temp5")
    prev_sixth_mem = read_memory(0xf0040000, 0x2000, "temp6")

    if peri_addr != 0x40023c10 and peri_addr != 0x40023c04 and peri_addr != 0xffffffff:
        if peri_addr % 0x4 == 0:
            if value < 0:
            ...
            str_val = format(1 << 8*(peri_addr % 4), '08x')
            str_val = reverse(str_val)
            str_val = codecs.decode(str_val, "hex")

            l.write_memory(int(peri_addr/4)*4, str_val, 4)

    #stepi(2000)

    next_first_mem = read_memory(base_addr, length, "temp1")
    next_second_mem = read_memory(0x50000000, 0x40000, "temp2")
    next_third_mem = read_memory(0xe0000000, 0x10000, "temp3")
    next_fourth_mem = read_memory(0xe0040000, 0x2000, "temp4")
    next_fifth_mem = read_memory(0xf0000000, 0x10000, "temp5")
    next_sixth_mem = read_memory(0xf0040000, 0x2000, "temp6")
...
def calc_init(peri_addr):
    ...
    peri_addr = int(peri_addr, 16)
    l = gdb.inferiors()[0]
    init_peri_val = read_memory(peri_addr, 4, "temp")
    init_peri_val = bytes_to_int(init_peri_val)
    fp = open("./result.txt", 'a')
    fp.write("addr 0x%x information\n" %peri_addr)
    fp.write("initial value 0x%x\n" %init_peri_val)
...
def add_prev_peri():
    ...
    while True:
        ...
        if addr != 0x40023c10 and addr != 0x40023c04 and addr != 0xffffffff:
            if addr % 4 == 0:
                if add_value < 0:
                ...
                str_val = format(add_value << 8*(addr % 4), '08x')
                str_val = reverse(str_val)
                str_val = codecs.decode(str_val, "hex")
                l.write_memory(int(addr/4)*4, str_val, 4)
