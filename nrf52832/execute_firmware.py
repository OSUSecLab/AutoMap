...
def arg_parser():
    ...
    parser.add_argument('--base_addr', type=int, default=0x0, help="the base address of firmware")
...
def debug_instruction(uc, address, size, user_data):
    ...
    global clock
    uc.mem_write(0xe000e018, int_to_hex_rev(clock))
    uc.mem_write(0xe0001004, int_to_hex_rev(clock))
    clock = clock + 0x100
    ...
    if flag == 1:
        ...
        for addr_dict in set_bits:
            for addr in addr_dict:
                ...
                new_value = value
                uc.mem_write(addr, codecs.decode(format(new_value, '08x'), "hex"))
...
def main():
    ...
    uc.emu_start(start_addr | 1, 0, 0, count=200000)
